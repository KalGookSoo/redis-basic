...
#### 문제 1
**문제**: TTL/만료에 대한 설명으로 가장 알맞은 것은?

**정답**: SET key value EX 60은 값 설정과 동시에 60초 TTL을 설정한다.

**해설**:
- 본 장에서 소개한 대로 SET 명령의 EX 옵션을 사용하면 값 설정과 TTL 지정이 동시에 이루어집니다.
- EXPIRE key 60은 초 단위 TTL을 설정하며, TTL은 서버 시계를 기준으로 계산됩니다.
...
**정답**: TLS를 활성화하고 신뢰할 수 있는 보안 그룹/방화벽으로 포트를 제한한다.
